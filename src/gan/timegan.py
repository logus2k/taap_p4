...
class TimeGAN:
    def __init__(
        ...
        self.embedder = self._build_rnn_network(output_dim=hidden_dim, input_dim=n_features, name="embedder")
        self.recovery = self._build_rnn_network(output_dim=n_features, input_dim=hidden_dim, name="recovery")
        self.generator = self._build_rnn_network(output_dim=hidden_dim, input_dim=hidden_dim, name="generator")
        self.supervisor = self._build_rnn_network(output_dim=hidden_dim, input_dim=hidden_dim, name="supervisor")
        self.discriminator = self._build_rnn_network(output_dim=1, input_dim=hidden_dim, name="discriminator")

    def _build_rnn_network(self, output_dim, input_dim, name="rnn_block"):
        inputs = keras.Input(shape=(self.seq_len, input_dim))
        x = inputs
        for i in range(self.num_layers):
            x = layers.GRU(self.hidden_dim, return_sequences=True, name=f"{name}_gru_{i+1}")(x)
        outputs = layers.TimeDistributed(layers.Dense(output_dim), name=f"{name}_out")(x)
    ...
    def _build_supervisor_trainer(self):
        ...
        x = H
        for i in range(self.num_layers):
            x = layers.GRU(self.hidden_dim, return_sequences=True, name=f"supervisor_train_gru_{i+1}")(x)
        H_hat = layers.TimeDistributed(layers.Dense(self.hidden_dim), name="supervisor_train_out")(x)
    ...
    def _build_discriminator_trainer(self):
        ...
        model = keras.Model([H_real, H_fake], [Y_real, Y_fake], name="discriminator_trainer")
        return model
    ...
    def _build_generator_trainer(self):
        ...
        model = keras.Model(Z, [Y_fake, X_hat], name="generator_trainer")
        return model
    ...
    def prepare_adversarial_models(self):
        ...
        self.d_optimizer = keras.optimizers.Adam(self.learning_rate)
        self.g_optimizer = keras.optimizers.Adam(self.learning_rate)

    @tf.function
    def train_adversarial_step(self, X_real):
        batch_size = tf.shape(X_real)[0]

        # Real latent representations
        H_real = self.embedder(X_real, training=False)

        # Random noise in latent space
        Z = tf.random.uniform(shape=(batch_size, self.seq_len, self.hidden_dim), minval=0.0, maxval=1.0)
        ...
        with tf.GradientTape() as g_tape:
            ...
            g_loss = g_loss_u + self.gamma * g_loss_v
    ...
    def fit(self, sequences, epochs=100, batch_size=128, verbose=1):
        ...
        dataset = tf.data.Dataset.from_tensor_slices(
            sequences.astype("float32")
        ).shuffle(min(len(sequences), 10000)).batch(batch_size)
        ...
        for epoch in range(epochs):
            epoch_d, epoch_g, epoch_gu, epoch_gv = [], [], [], []
    ...
    def generate(self, n_samples):
        Z = np.random.uniform(0, 1, size=(n_samples, self.seq_len, self.hidden_dim)).astype(np.float32)
